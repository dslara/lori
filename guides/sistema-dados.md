...
│  ├── insights.ts  → Análises consolidadas (v3.2.1)          │
...
/ul-data-analytics  # Analytics avançados
/ul-data-manage init  # Inicializar estrutura
/ul-data-manage reset # Resetar todos os dados (cuidado!)
```
...
2. Skills usadas são registradas em `session_skills.csv` (com campo `correct`)

### `/ul-data-status` e `/ul-data-analytics`
...
### insights.ts (Consolidada v3.2.1)

**Histórico**: Substitui 5 tools (`analytics.ts`, `effectiveness.ts`, `patterns.ts`, `weakness.ts`, `dashboard.ts`)

**Operações**:
- `getSummary` - Resumo geral (streak, tempo, foco)
- `getEffectiveness` - Efetividade por técnica
- `getPatterns` - Padrões de estudo (melhor período, duração ideal)
- `getWeaknesses` - Pontos fracos e técnicas sugeridas
- `getSRS` - Flashcards revisados/pendentes
- `getDifficultyLevel` - Nível adaptativo
- `generateReport` - Relatório completo consolidado
- `showDashboard` - Dashboard visual formatado
- `comparePeriods` - Comparar semanas/meses
...
1. **Use `/ul-data-status`** regularmente para acompanhar progresso
2. **Use `/ul-data-analytics`** semanalmente para análise detalhada
3. **Use `/ul-data-manage init`** se precisar resetar ou reconfigurar
4. **Use `/ul-data-backup`** antes de fazer mudanças grandes
5. **Consulte `data/schema.md`** para documentação técnica completa

---

*Sistema de dados do Ultralearning v3.0 — Powered by TypeScript Tools*
