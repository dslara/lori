...
//import { mkdirSync, appendFileSync, existsSync, promises } from "node:fs/promises"
...
const pluginFileDir = dirname(pluginFilePath)
...
async function loadSessionMap(): Promise<void> {
  ...
  try {
    ...
    const content = await promises.readFile(sessionMapPath, "utf-8")
    ...
    if (existsSync(sessionMapPath)) {
      ...
      await promises.rename(sessionMapPath, backupPath)
...
async function saveSessionMap(): Promise<void> {
  ...
  try {
    ...
    await promises.rename(tempPath, sessionMapPath)
