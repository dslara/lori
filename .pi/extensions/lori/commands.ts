...
Use /skill:lori-retro para ritual completo.`,
  ...
  pi.registerCommand("lori-resources", {
    ...
    handler: async (args, ctx) => {
      ...
      // Resolve file by number or name (used for direct arg or after selection)
      ...
      // Helper to open the selection overlay (allows returning from preview)
      async function openSelection() {
        await ctx.ui.custom<undefined>((tui, theme, _kb, done) => {
          ...
          selectList.onSelect = (item) => {
            ...
            // Defer preview so selection overlay closes first
      ...
      // Helper to show Markdown preview overlay
      ...
      // If arg provided, preview directly
      if (resolved) {
        await showPreview(resolved);
        return;
      }

      // Otherwise open interactive selection overlay
      await openSelection();
    },
  });
}
